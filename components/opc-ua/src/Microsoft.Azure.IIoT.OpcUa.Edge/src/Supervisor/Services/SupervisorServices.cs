    ...
    public class SupervisorServices : IActivationServices<string>, ISupervisorServices,
        ...
        public async Task DeactivateAsync(string id, CancellationToken ct) {
            ...
            try {
                if (!_twinHosts.TryGetValue(id, out twin)) {
                    _logger.Debug("{id} entity twin not running.", id);
                    return;
                }
                _twinHosts.Remove(id);
            }
        ...
        public async Task AttachAsync(string id, string secret) {
            ...
            try {
                ...
                _logger.Debug("Attaching entity {id} twin...", id);
                var host = new TwinHost(this, _config, id, secret, _logger);
                _twinHosts.Add(id, host);
        ...
        private class TwinHost : IDisposable, IProcessControl, IIoTEdgeConfig {
            ...
            /// Activation state
            /// </summary>
            public EntityActivationState Status { get; private set; }
